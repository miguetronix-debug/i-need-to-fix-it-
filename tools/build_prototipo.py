...
document.getElementById('titulo').textContent=
  ...
  // OJO con el selector: tiene que ser «.pchip[data-n]», no «.pchip».
  // El chip de «Casos por fallo» también es un .pchip pero NO lleva data-n, así
  // que con el selector amplio se le asignaba onclick=()=>irPaso(NaN), lo que
  // borraba su verCasos() y hacía que al pulsarlo no ocurriera absolutamente
  // nada: irPaso(NaN) sale por la primera línea sin decir ni pío.
  document.querySelectorAll('.pchip[data-n]').forEach(c=>{
    const n=+c.dataset.n;
    c.onclick=()=>irPaso(n);
  });
